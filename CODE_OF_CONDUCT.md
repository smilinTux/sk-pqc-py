...
We as members, contributors, and leaders pledge to make participation in the `sk-pqc`
...
Because `sk-pqc` is **cryptographic** software, positive conduct here also means:
...
- **Intellectual honesty about security claims.** Do not overstate assurance. We say
  ...
  "unbreakable." Claims cite evidence (surface + FIPS number + hybrid-vs-classical).
- **Good-faith review of crypto changes.** Combiner and wire-format changes affect
  every peer; engage with the cross-impl vector gate rather than around it.
...
maintainer contact listed on the [`smilinTux/sk-pqc-py`](https://github.com/smilinTux/sk-pqc-py)
