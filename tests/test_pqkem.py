"""sk_pqc — hybrid KEM (X25519 + ML-KEM-768) primitive tests.
...
Run from HOME::
    ...
    cd ~ && ~/.skenv/bin/python -m pytest <abs>/tests/test_pqkem.py -q
"""
...
# Locate the sk_pqc cross-impl vector. Prefer the env override (set by conftest
# to the bundled copy), else the known sibling Dart checkout path.
_VECTOR_CANDIDATES = [
    Path(p)
    for p in [
        os.environ.get("SK_PQC_VECTOR", ""),
        str(Path(__file__).parent / "vectors" / "hybrid_kem_x25519_mlkem768.json"),
        str(
            Path.home()
...
def test_registry_entry_is_active_hybrid():
    from sk_pqc.crypto_suites import SuiteKind, SuiteStatus, get_suite
...
def test_classical_fallback_is_not_silent():
    ...
    from sk_pqc.pqkem import PqKemError, PqKemUnavailable
